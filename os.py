...
# ==================== 2 ADMINS YAHAN DALO ====================
TOKEN = "8687442765:AAEtfUqA3XOW4ROrLmgA4gvSHJbIH7IdwW4"
ADMIN_IDS = [8723310707, 7983241359]  # <-- YAHAN APNE 2 ADMIN IDs DALO
# =============================================================
...
# ==================== USER FUNCTIONS (With ADMIN_IDS list) ====================
def is_user(uid):
    if uid in ADMIN_IDS:  # Changed from uid == ADMIN_ID
        return True
...
def can_attack(uid):
    ...
    if uid not in ADMIN_IDS:  # Changed from uid != ADMIN_ID
...
# ==================== ALL HANDLERS (With ADMIN_IDS list) ====================
...
def handle_attack(chat_id, uid, cid, args):
    ...
    if is_locked and uid not in ADMIN_IDS:  # Changed from uid != ADMIN_ID
...
def main():
    global last_id
    load()
    for admin in ADMIN_IDS:  # Add all admins
        add_user(admin, 365, 0)
    check_binary()
    
    print("""
    ╔══════════════════════════════════════╗
    ║   ⚡ DDOS BOT STARTED ⚡              ║
    ║   Multi-Admin Mode Active            ║
    ║   Owner: @TG_DEVILOP                 ║
    ╚══════════════════════════════════════╝
    """)
