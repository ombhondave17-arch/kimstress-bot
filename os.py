...
# ═══════════════════════════════════════════════════════════════
#                    BOT 2 CONFIGURATION (TG_DEVILOP)
# ═══════════════════════════════════════════════════════════════
...
def send_video(token, chat_id, video_id, caption):
    try:
...
def handle_attack_input(token, chat_id, uid, text, is_user_func, can_attack_func, add_attack_func, contact, users_data, add_slot_func):
    global is_attack, current_target
    
    parts = text.strip().split()
    if len(parts) != 3:
        send_msg(token, chat_id, "❌ Send: IP PORT TIME\nExample: 1.2.3.4 80 30")
        return
    ...
    try:
        ...
        if sec < 5 or sec > attack_time:
            send_msg(token, chat_id, f"❌ Time must be 5-{attack_time} seconds")
            return
    except:
        send_msg(token, chat_id, "❌ Invalid port or time")
        return
    
    can, msg = can_attack_func(uid)
    if not can:
        send_msg(token, chat_id, msg)
        return
    
    if get_slots() <= 0:
        send_msg(token, chat_id, f"❌ ALL {MAX_SLOTS} SLOTS BUSY!")
        return
    ...
    msg_id = send_msg_and_get_id(token, chat_id, f"""
⚡ ATTACK STARTED ⚡
━━━━━━━━━━━━━━━━━━━━━━
🎯 Target: {ip}
🔌 Port: {port}
...
""")
    if msg_id:
        threading.Thread(target=run_progress, args=(token, chat_id, msg_id, ip, port, sec)).start()
...
def handle_attack1(chat_id, uid, text):
    handle_attack_input(TOKEN1, chat_id, uid, text, is_user1, can_attack1, add_attack_count1, CONTACT1, users1, add_slot)
...
# ═══════════════════════════════════════════════════════════════
#                    BOT 2 FUNCTIONS (mirror of bot 1)
# ═══════════════════════════════════════════════════════════════
def is_user2(uid):
    if uid in ADMIN2:
        return True
...
def handle_attack2(chat_id, uid, text):
    handle_attack_input(TOKEN2, chat_id, uid, text, is_user2, can_attack2, add_attack_count2, CONTACT2, users2, add_slot)
...
def main():
    ...
    print("="*60)
    print(f"🤖 BOT 1: @TG_ROLEX (Two Admins: {ADMIN1})")
    print(f"🤖 BOT 2: @TG_DEVILOP (Single Admin: {ADMIN2})")
    print("="*60)
    ...
    while True:
        try:
            ...
            for update in data1.get("result", []):
                ...
                state = waiting1.get(chat_id)
                
                if state == "adduser":
                    handle_adduser1(chat_id, text)
                    waiting1.pop(chat_id, None)
            ...
            for update in data2.get("result", []):
                ...
                state = waiting2.get(chat_id)
                
                if state == "adduser":
                    handle_adduser2(chat_id, text)
                    waiting2.pop(chat_id, None)
                elif state == "removeuser":
